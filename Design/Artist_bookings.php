...
function OfferResponse(post_id) {
  window.location.href = 'OfferResponse.php?Status=' + post_id ;
}

function RejectOffer(post_id) {
  window.location.href = 'RejectOffer.php?Status=' + post_id ;
}
</script>
  </head>
  <body>
  <div class = "vignette">
    ...
    <ul class="snip1189">
        ...
        <li class="current"><a href="Artist_bookings.php">Booking Requests</a></li>
...
include('Connect.php');
$user = $_SESSION['Username'];
$sql3 = "SELECT * FROM Bookings where Artist = '$user'";
$res = $con->query($sql3);
    ...
    <div class="projcard projcard-blue">
      <div class="projcard-innerbox">
			<div class="projcard-textbox">
				<div class="projcard-title">Event: <?php echo  $row["EventName"] ?></div>
				<div class="projcard-subtitle">Hosted by: <?php echo  $row["Customer"] ?></div>
				<div class="projcard-bar"></div>
				<div class="projcard-description">Offer Amount: <?php echo  $row["Amount"]?> <br>Status:<a style="color: green; font-weight:bold;"> <?php echo  $row["Status"]?> </a></div>
        </div>
      </div>
    </div>
</div>
    ...
    <div class="projcard projcard-blue">
      <div class="projcard-innerbox">
			<div class="projcard-textbox">
				...
				<div class="projcard-subtitle">Hosted by: <?php echo  $row["Customer"] ?></div>
				<div class="projcard-bar"></div>
				<div class="projcard-description">Offer Amount: <?php echo  $row["Amount"]?></div>
        <button class="custom-btn btn-3" style="margin-top: -40px; position: absolute; left: 100px;" onclick="OfferResponse(<?php echo $row['BookingID'] ?>)" >Accept</button>
        <button class="custom-btn btn-3" style="margin-top: -40px; position: absolute; left: 230px;z-index: 9999;" onclick="RejectOffer(<?php echo $row['BookingID'] ?>)" >Decline</button>
        </div>
      </div>
    </div>
</div>
