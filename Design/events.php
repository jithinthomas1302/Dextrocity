...
<script type="text/javascript">
  ...
  <a href="new_event.html"><button class="custom-btn btn-5" ><span>NEW EVENT</span></button></a>  
</body></html>
<?php 
}
include('Connect.php');
$sql3 = "SELECT * FROM Events ORDER BY EventID DESC";
$res = $con->query($sql3);
...
<div class="projcard-container">
    ...
    <div class="projcard projcard-blue">
      ...
      <img class="projcard-img" src="<?php echo  $row["Image"] ?>" />
			<div class="projcard-textbox">
				<div class="projcard-title"><?php echo  $row["Name"] ?></div>
				<div class="projcard-subtitle">Posted by: <?php echo  $row["Host"] ?></div>
				<div class="projcard-bar"></div>
				<div class="projcard-description"><?php echo  $row["Description"] ?></div>
        <button class="custom-btn btn-3" onclick="showMorePosts(<?php echo $row['EventID'] ?> )">Show More</button>
        </div>
      </div>
    </div>
</div>
